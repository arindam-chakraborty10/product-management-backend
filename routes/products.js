...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if(isErr) return res.status(400).send('name and categoryId both are required..');
        ...
        console.log(flag);
        if(!flag) return res.status(400).send('Duplicate product found..');
        ...
        console.log(flag1);
        if(!flag1) return res.status(400).send('Duplicate product found in db..');
